...
/**
 * Extract gas consumption from forge test output
 * Looks for patterns like "[PASS] testSvgRenderer() (gas: 529463)"
 */
function extractGasFromForgeOutput(output: string): number {
    // Look for the pattern: (gas: NUMBER)
    const gasRegex = /\(gas:\s*(\d+)\)/;
    const match = output.match(gasRegex);
    
    if (match && match[1]) {
        return parseInt(match[1]);
    }
    
    // Fallback: try to get from NFT_GAS tags (though this often fails in view functions)
    ...
    // If neither method works, throw an error with helpful context
    throw new Error(
        'Could not extract gas consumption from forge output. ' +
        'Expected to find "(gas: NUMBER)" pattern or valid <NFT_GAS> tags.\n' +
        'Output snippet: ' + output.substring(0, 500) + '...'
    );
}
...
export function parseTestOutput(output: string): ParsedOutput {
    // Extract gas using the improved method
    let gas = extractGasFromForgeOutput(output);
